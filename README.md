...
└── titlepage.tex              # Настроенная титульная страница
```
...
2. Отредактируйте файлы `metadata.tex` и `content.tex` под свои нужды
...
### Классический метод размещения изображения с окружением `figure`
...
### Пользовательская команда `\insertimage` для размещения изображения

```tex
\insertimage[width=0.5\textwidth]{images/photo.jpg}{Изображение с измененным размером}
```

### Ручное размещение изображения с `\captionof`
...
### Центрирование изображения с ручной подписью
...
###  Горизонтальное расположение изображений в `minipage`
