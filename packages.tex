...
\renewcommand{\cftsecleader}{\cftdotfill{\cftdotsep}} % Добавляет точки для разделов первого уровня
\renewcommand{\cftbeforesecskip}{2pt}
\renewcommand{\cftbeforesubsecskip}{1pt}
\renewcommand{\cftdotsep}{1} % Уменьшает расстояние между точками
...
% Настройка пунктов списка источников
\makeatletter
\renewcommand{\@biblabel}[1]{#1.} % Изменяем формат меток
\makeatother